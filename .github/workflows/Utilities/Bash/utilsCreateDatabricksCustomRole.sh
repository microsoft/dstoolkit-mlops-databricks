...
if [ $Bool_Contains_DBX_Custom_Role_Exists == false ]; then
    echo "Is it..."
    cd MLOps_Engineer/DevOps_Pipelines/Utilities/RBAC_Role_Definition
    ls
    ...
    updateJson=$(jq -r --arg param_SubscriptionId "$SubscriptionId" ' .assignableScopes[0] = "/subscriptions/'$SubscriptonId'" ' DBX_Custom_Role.json) && echo -E "${updateJson}" > DBX_Custom_Role.json
