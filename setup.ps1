...
{
   $JsonData = Get-Content infrastructuredatabricks/databricks_configs/$environment/repos.json -raw | ConvertFrom-Json
   ...
   $JsonData | ConvertTo-Json -Depth 4  | set-content infrastructuredatabricks/databricks_configs/$environment/repos.json -NoNewline
}
 
foreach ($environment in $environments)
{
  $JsonData = Get-Content infrastructuredatabricks/databricks_configs/$environment/rbac.json -raw | ConvertFrom-Json
  ...
  $JsonData | ConvertTo-Json -Depth 4  | set-content infrastructuredatabricks/databricks_configs/$environment/rbac.json -NoNewline
}
